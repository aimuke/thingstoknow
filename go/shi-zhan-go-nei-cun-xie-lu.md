...
{% code title="golang_step_by_step/pprof/pprof/demo.go" %}
```go
package main
...
{% code title="golang_step_by_step/pprof/heap/demo2.go" %}
```go
// 展示内存增长和pprof，并不是泄露
package main
...
{% code title="golang_step_by_step/pprof/goroutine/leak_demo1.go" %}
```bash
// goroutine泄露导致内存泄露
package main
